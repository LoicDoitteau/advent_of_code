...
fs.readFile("./2018/day_24/part_2/input.txt", 'utf8', (err, input) => {
    var immuneAttackBoost = 1;
    while(true) {
        var[,,immuneInput,,infectionInput] = input.split(/(Immune System|\r\n\r\nInfection):\r\n/);
        var immuneGroups = immuneInput.split("\r\n").map(s => getGroup(s, 'immune', immuneAttackBoost));
        var infectionGroups = infectionInput.split("\r\n").map(s => getGroup(s, 'infection', 0));
        var i = 0;
        var flag = true
        while(immuneGroups.some(group => group.units > 0) && infectionGroups.some(group => group.units > 0)) {
            if(i++ == 1000000) {
                flag = false;
                break;
            }
            fight(immuneGroups, infectionGroups);
        }
        if(flag) {
            var winningGroups = immuneGroups.some(group => group.units > 0) ? immuneGroups : infectionGroups;
            if(winningGroups == immuneGroups) {
...
function getGroup(s, type, attackBoost) {
    ...
    return {type, units : Number(units), hitPoint : Number(hitPoint), weeknesses, immunities, attackDamage : Number(attackDamage) + attackBoost, attackType, initiative : Number(initiative)};
}
