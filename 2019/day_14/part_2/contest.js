...
fs.readFile("./2019/day_14/input.txt", 'utf8', (err, data) => {
    ...
    const tryGetMaxFuel = (from, step) => {
        let i = from;
        while(true) {
            const resources = getResources("FUEL", i);
            if(resources > MAX_ORE) return i - step;
            i += step;
        }
    }

    res = tryGetMaxFuel(tryGetMaxFuel(tryGetMaxFuel(tryGetMaxFuel(tryGetMaxFuel(tryGetMaxFuel(tryGetMaxFuel(1, 1000000), 100000), 10000), 1000), 100), 10), 1);
    console.log(res);
